...
export const makeGroupsSocket = (config) => {
    ...
    // Dedup concurrent groupMetadata calls for the same jid so N handlers
    // touching the same group at once produce 1 network query, not N.
    const inflightGroupMetadata = new Map();
    const fetchGroupMetadata = async (jid) => {
        ...
        for (const wait of backoffMs) {
            ...
            try {
                ...
                lastErr = err;
                // Only retry on 408 — other errors (not-authorized, 404) won't recover
    ...
    const groupMetadata = async (jid) => {
        const existing = inflightGroupMetadata.get(jid);
        if (existing)
            return existing;
        const promise = fetchGroupMetadata(jid).finally(() => {
            inflightGroupMetadata.delete(jid);
        });
        inflightGroupMetadata.set(jid, promise);
        return promise;
    };
