...
import { ACCOUNT_RESTRICTED_TEXT, aesDecryptCTR, aesEncryptGCM, CALL_ENDED_STATUSES, cleanMessage, Curve, decodeMediaRetryNode, decodeMessageNode, decryptMessageNode, decryptSecretEncryptedMessage, delay, derivePairingCodeKey, encodeBigEndian, encodeSignedDeviceIdentity, extractAddressingContext, extractE2ESessionFromRetryReceipt, getCallStatusFromNode, getHistoryMsg, getNextPreKeys, getStatusFromReceiptType, handleIdentityChange, hkdf, MISSING_KEYS_ERROR_TEXT, NACK_REASONS, NO_MESSAGE_FOUND_ERROR_TEXT, normalizeMessageContent, SERVER_ERROR_CODES, toNumber, unixTimestampSeconds, unpadRandomMax16, xmppPreKey, xmppSignedPreKey } from '../Utils/index.js';
import { makeMutex } from '../Utils/make-mutex.js';
import { makeOfflineNodeProcessor } from '../Utils/offline-node-processor.js';
import { makePasskeyFlow } from '../Utils/passkey-utils.js';
import { resolveCache } from '../Utils/shared-caches.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    /** shortcake/CRSC passkey companion-linking flow (see Utils/passkey-utils.js) */
    const passkeyFlow = makePasskeyFlow({ query, authState, ev, logger, browser: config.browser });
    /** this mutex ensures that each retryRequest will wait for the previous one to finish */
    const retryMutex = makeMutex();
    const msgRetryCache = config.msgRetryCounterCache ||
        resolveCache(config.sharedCaches, 'msgRetry', config.cacheId, () => new NodeCache({
            stdTTL: DEFAULT_CACHE_TTLS.MSG_RETRY, // 1 hour
            useClones: false
        }));
    const callOfferCache = config.callOfferCache ||
        resolveCache(config.sharedCaches, 'callOffer', config.cacheId, () => new NodeCache({
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                // passkey-locked accounts can emit this notice without the pairing payload (upstream #2681)
                if (!refBuffer || !primaryIdentityPublicKeyBuffer || !primaryEphemeralPublicKeyWrappedBuffer) {
                    logger.debug({ id: node.attrs.id }, 'skipping empty link code companion registration');
                    break;
                }
            ...
            case 'passkey_prologue_request':
                await passkeyFlow.handlePasskeyRequestNotification(node);
                break;
            case 'crsc_continuation':
                // handled async so the notification ack isn't blocked on the IQ round-trips
                passkeyFlow
                    .handleContinuationNotification(node)
                    .catch(error => ev.emit('pair-passkey.error', { error, continuation: true }));
                break;
        }
    ...
    // Build a 'call' event from a signalling node. `wrapper` = parent <call> attrs
    // (for wrapped stanzas) or the node's own attrs (for standalone stanzas).
    // Ported/extended from baron-baileys-v2: richer statuses, callKey decrypt,
    // mute, waiting room, negotiated codecs.
    const buildCallEvent = async (infoChild, wrapper) => {
        const status = getCallStatusFromNode(infoChild);
        const callId = infoChild.attrs['call-id'];
        const from = infoChild.attrs.from || infoChild.attrs['call-creator'];
        const call = {
            chatId: wrapper.from || from,
            from,
            callerPn: infoChild.attrs['caller_pn'],
            id: callId,
            date: wrapper.t ? new Date(+wrapper.t * 1000) : new Date(),
            offline: !!wrapper.offline,
            status
        };
        ...
        if (status === 'offer') {
            const videoNode = getBinaryNodeChild(infoChild, 'video');
            const audioNode = getBinaryNodeChild(infoChild, 'audio');
            call.isVideo = !!videoNode;
            call.isGroup = infoChild.attrs.type === 'group' || !!infoChild.attrs['group-jid'];
            call.groupJid = infoChild.attrs['group-jid'];
            if (audioNode?.attrs?.codec) {
                call.audioCodec = audioNode.attrs.codec;
            }
            if (videoNode?.attrs?.codec) {
                call.videoCodec = videoNode.attrs.codec;
            }
            // decrypt Signal-encrypted callKey (SRTP session key) from <enc> child
            ...
            if (encNode?.content) {
                try {
                    ...
                    const decrypted = await signalRepository.decryptMessage({ jid: from, type: encType, ciphertext });
                    const callMsg = proto.Message.decode(unpadRandomMax16(decrypted));
                    if (callMsg?.call?.callKey?.length) {
                        call.callKey = Buffer.from(callMsg.call.callKey);
                    }
                }
                catch (err) {
                    logger.debug({ err }, 'failed to decrypt call enc');
                }
            }
            if (callId) {
        ...
        // enrich signalling-only statuses
        if (status === 'peer_state') {
            const stateChild = getBinaryNodeChild(infoChild, 'peer_state');
            ...
            const vsChild = getBinaryNodeChild(infoChild, 'video_state');
            call.enabled = parseBool(vsChild?.attrs?.enabled ?? infoChild.attrs?.enabled);
        }
        else if (status === 'mute') {
            const muteChild = getBinaryNodeChild(infoChild, 'mute_v2');
            const muteAttrs = muteChild?.attrs || infoChild.attrs;
            call.muted = parseBool(muteAttrs?.muted ?? muteAttrs?.audio_muted);
            const audioChild = muteChild ? getBinaryNodeChild(muteChild, 'audio') : getBinaryNodeChild(infoChild, 'audio');
            if (audioChild?.attrs?.muted !== undefined) {
                call.muted = parseBool(audioChild.attrs.muted);
            }
        }
        if (callId && CALL_ENDED_STATUSES.has(status)) {
    ...
    const handleCall = async (node) => {
        try {
            ...
            const call = await buildCallEvent(infoChild, attrs);
            ev.emit('call', [call]);
        }
        catch (error) {
    ...
    // <mute_v2>, … each with a call-id) instead of wrapped in <call>. Emit a 'call' event
    // for recognised state stanzas and always ack (else WA keeps redelivering).
    const CALL_STATE_TAGS = new Set([
        'offer', 'offer_notice', 'terminate', 'accept', 'reject', 'preaccept', 'accept_ack',
        'enc-rekey', 'enc_rekey', 'peer_state', 'group_info', 'video_state', 'video_state_ack',
        'flow_control', 'mute_v2', 'waiting_room_request'
    ]);
    const handleStandaloneCallStanza = async (node) => {
        try {
            if (CALL_STATE_TAGS.has(node.tag)) {
                const call = await buildCallEvent(node, node.attrs);
                ev.emit('call', [call]);
            }
        }
        catch (error) {
            logger.error({ error, node: binaryNodeToString(node) }, 'error in handling standalone call stanza');
        }
        finally {
            await sendMessageAck(node).catch(ackErr => logger.error({ ackErr }, 'failed to ack call stanza'));
        }
    };
    ...
    // top-level call signalling stanzas (some accounts get these instead of wrapped <call>)
    for (const callTag of [
        'offer', 'offer_notice', 'terminate', 'accept', 'reject', 'preaccept', 'accept_ack',
        'enc-rekey', 'enc_rekey', 'peer_state', 'group_info', 'video_state', 'video_state_ack',
        'flow_control', 'transport', 'video', 'duration', 'mute_v2', 'lobby', 'heartbeat',
        'relaylatency', 'link_query', 'waiting_room_request'
    ]) {
    ...
    return {
        ...
        messageRetryManager,
        sendPasskeyResponse: passkeyFlow.sendPasskeyResponse,
        sendPasskeyConfirmation: passkeyFlow.sendPasskeyConfirmation
    };
};
