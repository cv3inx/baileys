...
export const makeMessagesRecvSocket = (config) => {
    ...
    /** keyed per-JID so retryRequests to different chats run in parallel */
    ...
    const handleReceipt = async (node) => {
        ...
        try {
            await Promise.all([
                receiptMutex.mutex(remoteJid || 'global', async () => {
    ...
    const handleNotification = async (node) => {
        ...
        try {
            await Promise.all([
                notificationMutex.mutex(remoteJid || 'global', async () => {
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(msg.key?.remoteJid || 'global', async () => {
                await decrypt();
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    ...
                    if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
                        ...
                        await retryMutex.mutex(msg.key?.remoteJid || 'global', async () => {
                            try {
                                if (!ws.isOpen) {
    ...
    ws.on('CB:message', (node) => {
        processNode('message', node, 'processing message', handleMessage).catch(err => onUnexpectedError(err, 'CB:message'));
    });
    ws.on('CB:call', (node) => {
        processNode('call', node, 'handling call', handleCall).catch(err => onUnexpectedError(err, 'CB:call'));
    });
    ws.on('CB:receipt', (node) => {
        processNode('receipt', node, 'handling receipt', handleReceipt).catch(err => onUnexpectedError(err, 'CB:receipt'));
    });
    ws.on('CB:notification', (node) => {
        processNode('notification', node, 'handling notification', handleNotification).catch(err => onUnexpectedError(err, 'CB:notification'));
    });
    ...
    ev.on('call', ([call]) => {
        ...
        })().catch(err => onUnexpectedError(err, 'call event'));
    });
