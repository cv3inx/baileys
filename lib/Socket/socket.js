...
import { resolveCachePreset } from '../Utils/shared-caches.js';
...
export const makeSocket = (config) => {
    ...
    const signalRepository = makeSignalRepository({ creds, keys }, logger, pnFromLIDUSync, {
        sharedCaches: config.sharedCaches,
        cacheId: config.cacheId,
        preset: resolveCachePreset(config.cachePreset)
    });
    let lastDateRecv;
    let epoch = 1;
    let keepAliveReq;
    ...
    const onMessageReceived = async (data) => {
        await noise.decodeFrame(data, frame => {
            // reset ping timeout and miss counter
    ...
    const end = async (error) => {
        ...
        try {
            ...
            logger.error({ err }, 'error emitting final connection.update');
        }
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        // scale dead threshold with interval to avoid false positives on slow networks
        const deadThresholdMs = keepAliveIntervalMs * 1.5 + 5000;
        if (diff > deadThresholdMs) {
            missedPongs++;
            // require 2 consecutive missed intervals before declaring connection lost
            if (missedPongs >= 2) {
                void end(new Boom('Connection was lost', { statusCode: DisconnectReason.connectionLost }));
            }
            else {
                logger.warn({ diff, missedPongs }, 'keepalive miss, waiting for next interval');
            }
        }
        else if (ws.isOpen && !pingInFlight) {
            missedPongs = 0;
            pingInFlight = true;
            query({
                tag: 'iq',
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // only go active if markOnlineOnConnect is true; staying passive keeps us
            // invisible to contacts and lets the phone receive push notifications (#2553)
            if (config.markOnlineOnConnect !== false) {
    ...
    const fetchNewChatMessageCap = async () => {
        try {
            return await executeWMexQuery({ input: { type: 'INDIVIDUAL_NEW_CHAT_MSG' } }, QueryIds.MESSAGE_CAPPING_INFO, XWAPaths.xwa2_message_capping_info, query, generateMessageTag);
        }
        catch (err) {
            // MEX endpoint returns 500 intermittently (#2539); treat as non-fatal
            logger.warn({ err }, 'fetchNewChatMessageCap failed, ignoring');
            return undefined;
        }
    };
    return {
