...
export const makeSocket = (config) => {
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        // Tolerate transient network stalls: require 2 consecutive misses
        // (~2x keepAliveIntervalMs of silence) before declaring the link dead.
        if (diff > keepAliveIntervalMs + 5000) {
            keepAliveMisses++;
            logger.warn({ diff, misses: keepAliveMisses }, 'keep alive stale');
    ...
    ws.on('CB:failure', (node) => {
        const reason = +(node.attrs.reason || DisconnectReason.connectionClosed);
