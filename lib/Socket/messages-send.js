...
import { buildMergedTcTokenIndexWrite, computeCsToken, isTcTokenExpired, resolveIssuanceJid, resolveTcTokenJid, shouldSendNewTcToken, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            else if (!isGroup && !isRetryResend && !isStatus && isLid && authState.creds.nctSalt?.length) {
                // cstoken (NCT) fallback: used when no tctoken for a 1:1 LID conversation
                try {
                    const csToken = computeCsToken(authState.creds.nctSalt, destinationJid);
                    stanza.content.push({
                        tag: 'cstoken',
                        attrs: {},
                        content: csToken
                    });
                    logger.trace({ jid: destinationJid }, 'attached cstoken fallback');
                }
                catch (err) {
                    logger.warn({ err }, 'failed to compute cstoken');
                }
            }
    ...
    const getMediaType = (message) => {
        if (message.imageMessage) {
            ...
            if (message.buttonsMessage.imageMessage) return 'image';
            if (message.buttonsMessage.videoMessage) return 'video';
            if (message.buttonsMessage.documentMessage) return 'document';
            if (message.buttonsMessage.locationMessage) return 'location';
            return 'buttons';
        }
        else if (message.interactiveMessage?.header) {
            const h = message.interactiveMessage.header;
            if (h.imageMessage) return 'image';
            if (h.videoMessage) return 'video';
            if (h.documentMessage) return 'document';
        }
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(() => {
                        messageMutex.mutex(fullMsg.key?.remoteJid || 'global', () => upsertMessage(fullMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                    });
                }
                for (const id of jid) {
                    ...
                    if (config.emitOwnEvents) {
                        process.nextTick(() => {
                            messageMutex.mutex(mentionMsg.key?.remoteJid || 'global', () => upsertMessage(mentionMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                        });
                    }
                    await delay(delayMs);
                }
                ...
                if ('album' in content) {
                    ...
                    for (const albumMedia of content.album) {
                        ...
                        if (config.emitOwnEvents) {
                            process.nextTick(() => {
                                messageMutex.mutex(albumMsg.key?.remoteJid || 'global', () => upsertMessage(albumMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                            });
                        }
                        await delay(delayMs);
                    }
