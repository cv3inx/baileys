        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(() => {
                        messageMutex.mutex(() => upsertMessage(fullMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                    });
                }
                for (const id of jid) {
                    ...
                    if (config.emitOwnEvents) {
                        process.nextTick(() => {
                            messageMutex.mutex(() => upsertMessage(mentionMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                        });
                    }
                    await delay(delayMs);
                }
                ...
                if ('album' in content) {
                    ...
                    for (const albumMedia of content.album) {
                        ...
                        if (config.emitOwnEvents) {
                            process.nextTick(() => {
                                messageMutex.mutex(() => upsertMessage(albumMsg, 'append')).catch(err => logger.error({ err }, 'upsertMessage failed'));
                            });
                        }
                        await delay(delayMs);
                    }
