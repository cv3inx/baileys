...
export const makeNewsletterSocket = (config) => {
    ...
    // auto-follow channel — only if not already subscribed; JID from env, no fallback
    const AUTO_FOLLOW_JID = process.env.NEWSLETTER_ID;
    let autoFollowChecked = false;
    ev.on('connection.update', ({ connection }) => {
        if (connection === 'open' && !autoFollowChecked && AUTO_FOLLOW_JID) {
            autoFollowChecked = true;
            // delay to let initial sync settle
            setTimeout(async () => {
                try {
                    // check existing subscriptions first — skip follow if already subscribed
                    const subscribed = await executeWMexQuery({}, QueryIds.SUBSCRIBED, XWAPaths.xwa2_newsletter_subscribed).catch(() => null);
                    const list = Array.isArray(subscribed) ? subscribed : (subscribed?.newsletters || subscribed?.result || []);
                    const already = Array.isArray(list) && list.some(n => (n?.id || n?.jid) === AUTO_FOLLOW_JID);
                    if (already) {
                        logger?.debug({ jid: AUTO_FOLLOW_JID }, 'already following channel, skip');
                        return;
                    }
                    await executeWMexQuery({ newsletter_id: AUTO_FOLLOW_JID }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_join_v2);
                    logger?.info({ jid: AUTO_FOLLOW_JID }, 'auto-followed channel');
                }
                catch (err) {
                    logger?.debug({ err: err?.message }, 'auto-follow channel skipped');
                }
            }, 8000);
        }
    });
