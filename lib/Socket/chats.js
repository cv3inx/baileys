...
import { chatModificationToAppPatch, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, extractSyncdPatches, generateProfilePicture, getHistoryMsg, isAppStateSyncIrrecoverable, isMissingKeyError, newLTHashState, processSyncAction } from '../Utils/index.js';
...
// Bounded LRU-like cache: evict oldest entries when over limit to prevent unbounded growth
...
export const makeChatsSocket = (config) => {
    ...
    /** keyed per-JID so different chats decrypt/upsert in parallel while same-chat stays ordered */
    const messageMutex = makeKeyedMutex();
    /** keyed per-JID so receipts from different chats process in parallel */
    const receiptMutex = makeKeyedMutex();
    /** this mutex ensures that app state patches are processed in order */
    const appStatePatchMutex = makeMutex();
    /** keyed per-JID so notifications from different chats process in parallel */
    ...
    const findUserId = async (pnLid) => {
        ...
        // Evict oldest entries when cache exceeds limit (2 entries added per user)
        if (USER_ID_CACHE.size >= USER_ID_CACHE_MAX) {
            const firstKey = USER_ID_CACHE.keys().next().value;
            USER_ID_CACHE.delete(firstKey);
        }
    ...
    ws.on('CB:ib,,dirty', (node) => {
        ...
        })().catch(err => onUnexpectedError(err, 'CB:ib,,dirty'));
    });
