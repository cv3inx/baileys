...
export const makeChatsSocket = (config) => {
    ...
    const profilePictureUrl = async (jid, type = 'image', timeoutMs = 5000, shouldIncludeTcToken = false) => {
        ...
        // tctoken must be nested inside <picture>, not a sibling (#2589)
        // gated behind shouldIncludeTcToken — off by default avoids 408 timeouts
        let pictureChildren = undefined;
        if (shouldIncludeTcToken && serverProps.profilePicPrivacyToken && isUserJid && !isSelf) {
            const tcTokenNode = await buildTcTokenFromJid({
                authState,
                jid: normalizedJid,
                getLIDForPN
            });
            if (tcTokenNode) pictureChildren = [tcTokenNode];
        }
        const content = [{ tag: 'picture', attrs: { type, query: 'url' }, content: pictureChildren }];
    ...
    const presenceSubscribe = async (toJid) => {
        ...
        return sendNode({
            ...
            content: tcTokenNode ? [tcTokenNode] : undefined
        });
    };
