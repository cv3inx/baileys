...
export const DEFAULT_CONNECTION_CONFIG = {
    ...
    /**
     * Shared cache pools from `makeSharedCacheBundle()`, for running many
     * sockets in one process. When set, `cacheId` must be set too (a stable
     * per-bot id) so each socket gets its own namespace in the pools.
     * Cuts empty-cache overhead from ~521 KB/socket to ~5 KB/socket.
     */
    sharedCaches: undefined,
    /** Stable unique id for this socket, used to namespace `sharedCaches`. */
    cacheId: undefined,
    /**
     * Per-socket cache sizing: 'tiny' | 'small' | 'medium' | 'large', or an
     * object overriding individual slots. Only applies to caches that are NOT
     * served from `sharedCaches`. Defaults to 'large' (pre-0.4 sizing).
     */
    cachePreset: undefined
};
