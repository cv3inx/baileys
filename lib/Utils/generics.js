...
export const generateMessageIDV2 = (userId) => {
   ...
   return baseId.slice(0, pos) +baseId.slice(pos);
};
...
export const getErrorCodeFromStreamError = (node) => {
    ...
    // Only use badSession (500) for explicit logout/session errors.
    // Unknown stream errors default to connectionClosed (428) so they are
    // treated as recoverable — prevents unnecessary session deletion.
    const BAD_SESSION_REASONS = new Set(['bad_session', 'bad_session_description', 'account_banned']);
    const defaultCode = BAD_SESSION_REASONS.has(reason)
        ? DisconnectReason.badSession
        : DisconnectReason.connectionClosed;
    const statusCode = +(node.attrs.code || CODE_MAP[reason] || defaultCode);
