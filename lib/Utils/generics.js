...
export const generateParticipantHashV2 = (participants) => {
    ...
    const sha256Hash = sha256(Buffer.from(sorted.join(''))).toString('base64');
...
export const generateMessageIDV2 = (userId) => {
    ...
    return '3EB0' + hash.toString('hex', 0, 9).toUpperCase();
};
...
// numeric <signal type="N"/> → status (WA call signalling); ported from baron-baileys-v2
const CALL_SIGNAL_TYPE_STATUS = {
    '12': 'mute',
    '13': 'preaccept',
    '15': 'video_state',
    '17': 'group_info',
...
const CALL_TERMINATE_REASON_STATUS = {
    timeout: 'timeout',
    Timeout: 'timeout',
    RejectDoNotDisturb: 'reject_do_not_disturb',
...
export const getCallStatusFromNode = ({ tag, attrs }) => {
    switch (tag) {
        ...
        case 'offer_notice':
            return 'offer';
        case 'terminate':
            return CALL_TERMINATE_REASON_STATUS[attrs.reason] || 'terminate';
        case 'preaccept':
            return 'preaccept';
        case 'transport':
            return 'transport';
        ...
        case 'video_state_ack':
            return 'video_state_ack';
        case 'flow_control':
            return 'flow_control';
        case 'mute_v2':
            return 'mute';
        case 'waiting_room_request':
            return 'waiting_room_request';
        case 'signal':
            return CALL_SIGNAL_TYPE_STATUS[attrs.type] || 'ringing';
        default:
            return 'ringing';
    }
};
// call statuses that mean the call is over → drop cached offer
export const CALL_ENDED_STATUSES = new Set([
    'reject',
    'accept',
    'timeout',
    'terminate',
    'reject_do_not_disturb',
