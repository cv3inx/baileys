            ...
            return cacheMutex.runExclusive(async () => {
                ...
                for (const type in data) {
                    for (const id in data[type]) {
                        ...
                        // A null/undefined value means "delete this key" — that is how
                        // callers remove sessions and pre-keys, and how store.set()
                        // below interprets it. Caching the null instead made the
                        // deleted key look like a live cache hit whose value was null,
                        // so a later get() returned null rather than falling through
                        // to the store. Evict instead.
                        if (value == null) {
