        ...
        createBufferedFunction(work) {
            return async (...args) => {
                ...
                try {
                    ...
                    // Reuses the single tracked timer: this used to allocate an
                    // untracked setTimeout per call, so destroy() couldn't cancel
                    // them and a busy socket kept hundreds of live timers. At
                    // fleet scale that alone churned the event loop.
    ...
    switch (event) {
        ...
        case 'messages.delete':
            ...
            if ('keys' in deleteData) {
                ...
                // jid-based clear-chat: drop all buffered messages for that chat
...
const stringifyMessageKey = (key) => {
    // include participant for newsletter messages to prevent key collision on burst traffic (#2464)
    const extra = key.participant ? `,${key.participant}` : '';
    return `${key.remoteJid},${key.id},${key.fromMe ? '1' : '0'}${extra}`;
};
