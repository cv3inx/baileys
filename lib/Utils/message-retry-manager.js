...
import { CACHE_PRESETS } from './shared-caches.js';
...
export class MessageRetryManager {
    /**
     * @param preset optional cache sizing (see CACHE_PRESETS). Defaults to the
     *   pre-0.4 sizes. `ttlAutopurge` is off throughout: it schedules a timer per
     *   entry, which at fleet scale means thousands of timers churning the event
     *   loop. Expiry is still enforced lazily on read, and `max` bounds growth.
     */
    constructor(logger, maxMsgRetryCount, preset = CACHE_PRESETS.large) {
        this.logger = logger;
        this.recentMessagesMap = new LRUCache({
            max: preset.recentMessages ?? RECENT_MESSAGES_SIZE,
            ttl: 5 * 60 * 1000,
            ttlAutopurge: false,
            dispose: (_value, key) => {
        ...
        // These three were unbounded: with only a TTL, a socket taking sustained
        // retry traffic grew them without limit until the TTL happened to fire.
