        ...
        state: {
            ...
            keys: {
                get: async (type, ids) => {
                    ...
                    // Batch read: one `id IN (...)` query instead of one query per id.
                    // Baileys calls get() with many jids at once (e.g. assertSessions on
                    // large groups) — N round-trips to SQLite becomes 1.
                    const placeholders = ids.map(() => '?').join(',');
                    const rows = db
                        .prepare(`SELECT id, value FROM signal_keys WHERE type = ? AND id IN (${placeholders})`)
                        .all(type, ...ids);
