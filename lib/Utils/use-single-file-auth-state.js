    ...
    const loadKey = async () => {
        return await mutex.runExclusive(async () => {
            ...
            try {
                ...
                // ENOENT = new session, anything else = warn
                if (err?.code !== 'ENOENT') {
                    console.warn('[baileys] useSingleFileAuthState: failed to read auth file:', err?.message);
                }
                fileData = {};
            }
            isLoaded = true;
        ...
        flushTimeout = setTimeout(async () => {
            ...
            await mutex.runExclusive(async () => {
                try {
                    ...
                    console.warn('[baileys] useSingleFileAuthState: failed to flush auth file:', err?.message);
                }
            });
        }, FLUSH_TIMEOUT_MS);
    };
