...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
        ...
        for (let i = 0; i < uniqueHosts.length; i++) {
            ...
            logger.debug({ hostname, mediaType, url: url.split('?')[0] }, `uploading to host (${i + 1}/${uniqueHosts.length})`);
            let result;
            try {
                ...
                if (result?.url || result?.direct_path) {
                    ...
                    // 401 = auth token expired → force-refresh before next host
                    if (status === 401 || status === 403) {
                        try { uploadInfo = await refreshMediaConn(true); } catch { }
                    }
                    throw new Error(`upload failed, status=${status ?? 'unknown'}, reason: ${JSON.stringify(result)}`);
                }
            }
            catch (error) {
                logger.warn({ trace: error?.stack, uploadResult: result, status: result?._status }, `Error uploading to ${hostname}${isLast ? ' (last host, giving up)' : ', retrying next host...'}`);
            }
        }
        if (!urls) {
            throw new Boom(`Media upload failed on all hosts (${uniqueHosts.map(h => h.hostname).join(', ')})`, { statusCode: 500 });
        }
        return urls;
    };
};
