    ...
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
        ...
        for (let i = 0; i < hosts.length; i++) {
            ...
            try {
                ...
                logger.warn({ trace: error?.stack, uploadResult: result }, `Error uploading to ${hostname}${isLast ? ' (last host, giving up)' : ', retrying next host...'}`);
            }
        }
        if (!urls) {
            throw new Boom(`Media upload failed on all hosts (${hosts.map(h => h.hostname).join(', ')})`, { statusCode: 500 });
        }
        return urls;
    };
};
