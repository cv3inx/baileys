...
import { generateMsgSecretKey } from './reporting-utils.js';
...
const getSecretEncTypeScopes = (secretEncType) => {
...
export const decryptSecretEncryptedMessage = async (message, messageSecret, meId, meLid, logger) => {
    ...
    if (!secretEncTypeScopes) {
        logger?.warn({ secretEncType: secretEncryptedMessage.secretEncType, targetMessageKey: secretEncryptedMessage.targetMessageKey }, 'unsupported secret encrypted message type');
        return;
    }
    if (!targetMessageKey?.id) {
        logger?.warn({ targetMessageKey: secretEncryptedMessage.targetMessageKey }, 'missing secret encrypted message target');
        return;
    }
    ...
    for (const secretEncTypeScope of secretEncTypeScopes) {
        try {
            const decryptKey = generateMsgSecretKey(secretEncTypeScope, targetMessageKey.id, originalSender, modificationSender, messageSecret);
    ...
    if (!decryptedMessage) {
        logger?.warn({ err: decryptErr, secretEncType: secretEncryptedMessage.secretEncType, targetMessageKey, messageKey: message.key, originalSender, modificationSender }, 'failed to decrypt secret encrypted message');
        return;
    }
...
export const isRealMessage = (message) => {
    // stub-only real messages (e.g. missed calls, group add) have no message content
    if (REAL_MSG_STUB_TYPES.has(message.messageStubType))
        return true;
    if (REAL_MSG_REQ_ME_STUB_TYPES.has(message.messageStubType) && !message.key.fromMe)
        return true;
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        // Security guard: these protocol message types are only valid from our own devices.
        // Drop silently if arriving from a non-self sender — prevents spoofing local state.
        ...
        if (protocolMsg.type !== null &&
            ...
            !message.key.fromMe) {
            logger?.debug({ type: protocolMsg.type, key: message.key }, 'dropping self-only protocolMessage from non-self sender');
            return;
        }
            ...
            case proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
                ...
                if (process) {
                    ...
                    if (histNotification.syncType !== proto.HistorySync.HistorySyncType.ON_DEMAND) {
                        const existing = creds.processedHistoryMessages || [];
                        const updated = [...existing, { key: message.key, messageTimestamp: message.messageTimestamp }];
                        ev.emit('creds.update', {
                            processedHistoryMessages: updated.length > 50 ? updated.slice(-50) : updated
                        });
                    }
        ...
        if (pollMsg) {
            ...
            if (!pollEncKey) {
                logger?.warn({ creationMsgKey }, 'poll vote: missing messageSecret on poll creation message');
            }
            else {
                try {
