...
const prepareStickerPackMessage = async (message, options) => {
    ...
    const hasJimp = 'jimp' in lib && !!lib.jimp?.default;
    ...
    try {
        ...
        if (hasSharp) {
            ...
            thumbnailBuffer = await jimpImage.resize(252, 252).getBufferAsync(Jimp.MIME_JPEG);
        }
        else {
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Note --- caller-supplied messageSecret (#2552). Per-type branches (poll, event) keep precedence.
